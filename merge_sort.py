...
class Renderer(Window):
    def __init__(self):
        ...
        self.batch = Batch()
        self.num_bars = 200  # Increase the number of bars
        self.x = random.sample(range(1, self.num_bars + 1), self.num_bars)
        self.bars = [Rectangle(20 + e * 4, 60, 3, i*3, batch=self.batch, color=(0, 0, 255, 255)) for e, i in enumerate(self.x)]
...
renderer = Renderer()
clock.schedule_interval(renderer.on_update, 0.0001)
run()
